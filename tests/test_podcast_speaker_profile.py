...
class TestMigration20:
    ...
    def test_migration_is_registered_in_manager(self):
        ...
        assert len(manager.migrations) >= 20
        assert all(m.down_sql for m in manager.migrations)
        assert "speaker_config" in manager.migrations[19].sql
        assert "option<record<speaker_profile>>" in manager.migrations[19].sql
        assert "speaker_config.name" in (manager.migrations[19].down_sql or "")
