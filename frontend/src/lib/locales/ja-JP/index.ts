export const jaJP = {
  ...
  settings: {
    ...
    simple: "シンプル",
    helpMeChoose: "選び方",
    docHelp: "・自動（推奨）はPDFやOfficeドキュメントにビジョンモデルを使用し、テキスト、表、画像を優れた精度で抽出します。・シンプルは高度なフォーマットなしで基本的なテキストコンテンツを抽出します。",
    firecrawl: "Firecrawl",
    jina: "Jina",
  ...
  models: {
    ...
    visionModelLabel: "ビジョンモデル",
    visionModelDesc: "画像や動画フレームの分析に使用（マルチモーダルモデル：GPT-4V、Claude 3、Gemini等）",
  ...
  apiKeys: {
    ...
    vertexConfigNote: "Google Vertex AI にはGCPプロジェクトとサービスアカウントが必要です。セキュリティのため、環境変数で設定することを推奨します。",
    ...
    defaultModelHint: "デフォルトで使用するモデル（例：gpt-4、gpt-3.5-turbo）",
    noConfigs: "設定がありません",
