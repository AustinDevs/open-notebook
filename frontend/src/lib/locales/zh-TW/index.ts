export const zhTW = {
  ...
  settings: {
    ...
    simple: "Simple",
    helpMeChoose: "幫助我選擇",
    docHelp: "· 自動 (推薦): 使用視覺模型處理 PDF 和 Office 文件，可以出色地提取文字、表格和圖片。 · Simple: 從文件中提取基本文字內容，不進行進階格式化。",
    firecrawl: "Firecrawl",
    jina: "Jina",
  ...
  models: {
    ...
    visionModelDesc: "用於分析圖片和影片幀（任何多模態模型：GPT-4V、Claude 3、Gemini 等）",
  ...
  apiKeys: {
    ...
    vertexConfigNote: "Google Vertex AI 需要 GCP 專案和服務帳戶。建議通過環境變數配置以獲得更好的安全性。",
    ...
    baseUrlHint: "預設：{url}",
    defaultModel: "預設模型",
    defaultModelHint: "預設使用的模型（例如：gpt-4、gpt-3.5-turbo）",
    noConfigs: "暫無設定",
