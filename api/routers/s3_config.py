"""API router for per-user S3 storage configuration."""
...
async def save_s3_config(request: S3ConfigRequest):
    """Save S3 configuration to the current user's database."""
    try:
        # Create or update config (upsert handled by save())
        config = S3Config(
            access_key_id=SecretStr(request.access_key_id),
            secret_access_key=SecretStr(request.secret_access_key),
        ...
        # Set credentials for the current request's ContextVar
        set_s3_credentials_for_request(
            {
...
async def delete_s3_config():
    """Remove S3 configuration from the current user's database."""
    try:
        config = await S3Config.get_for_user()
        if config:
            await config.delete()
            # Clear credentials for this request
            set_s3_credentials_for_request(None)
