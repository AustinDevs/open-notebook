...
async def content_process(state: SourceState) -> dict:
    ...
    if file_path:
        ...
        if content_type == "image":
            ...
            # Restore original S3 path and filename in result
            if original_file_path and original_file_path.startswith("s3://"):
                result["content_state"].file_path = original_file_path
                # Use original filename as title if not set or using temp filename
                if original_filename and (
                    not result["content_state"].title
                    or result["content_state"].title.startswith("tmp")
                    or "tmp" in result["content_state"].title.lower()
                ):
                    result["content_state"].title = os.path.splitext(original_filename)[0]
            return result
            ...
            result = await process_pdf_content(content_state, file_path)
            # Clean up temp file if we created one
            if temp_file_path:
                try:
                    os.unlink(temp_file_path)
            ...
            result = await process_office_content(content_state, file_path)
            # Clean up temp file if we created one
            if temp_file_path:
                try:
                    os.unlink(temp_file_path)
